...
export function TurnCard({
  ...
  const summary = summarizeEvents(turn.events);
  ...
  return (
    <div className="border-b border-black/5 pb-3 last:border-b-0 dark:border-white/5">
      <button onClick={() => setOpen((v) => !v)} className="flex w-full items-start gap-2 py-2 text-left">
        <span className="mt-1 shrink-0 text-zinc-400">{open ? "▾" : "▸"}</span>
        <span className="min-w-0 flex-1">
          <span
            className={`block font-medium text-zinc-900 dark:text-zinc-50 ${open ? "" : "line-clamp-3"}`}
          >
